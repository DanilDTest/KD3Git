...
show_menu() {
    ...
    echo -e "${GREEN}4. ${BLUE}Обновить правила по рабочей базе${NC}"
  
    echo -e "\n${GREEN}Служебные скрипты:${NC}"
    echo -e "${GREEN}5. ${BLUE}Переименовать слитые ветви${NC}"
    echo -e "${GREEN}6. ${BLUE}Добавить/обновить репозиторий к ДЗ Гит${NC}"
...
while true; do
    ...
    case $choice in
        ...
        4) run_script "ДляВызова/ОбновитьПравилаПоРабочейБазе.sh" ;;
        5) run_script "ДляВызова/Служебные/ПереименоватьСлитыеВетки.sh" ;;
        6) run_script "ДляВызова/Служебные/ДобавитьОбновитьРепозиторийКД3Гит.sh" ;;
        0) exit 0 ;;
